              ...
              {!collapsed && item.submenu && expandedItems.has(item.id) && (
                <div className='ml-8 mt-2 space-y-1'>
                  {item.submenu.map((subitem) => {
                    return (
                      <button className='w-full text-left p-2 text-sm text-slate-600 dark:text-slate-400 dark:hover:text-slate-200 dark:hover-bg-slate-800/50 rounded-lg transition-all'>
                        {subitem.label}
                      </button>
                    )
                  })}
